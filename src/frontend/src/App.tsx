...
const ALL_PROMPTS: PromptDef[] = [
  ...
  {
    icon: "⭐",
    text: "What clubs and activities are available?",
    category: "Activities",
  },
  {
    icon: "🍽️",
    text: "What's on the lunch menu this week?",
    category: "Daily Life",
  },
  {
    icon: "🏫",
    text: "Tell me about the school facilities",
    category: "School",
  },
  ...
  {
    icon: "📞",
    text: "How do I contact the admissions team?",
    category: "Admissions",
  },
  { icon: "🎽", text: "What sports are offered?", category: "Activities" },
  {
    icon: "🎭",
    text: "Tell me about the arts programme",
    category: "Activities",
  },
];
...
function sortedPrompts(counts: Record<string, number>): PromptDef[] {
  return [...ALL_PROMPTS].sort(
    (a, b) => (counts[b.text] || 0) - (counts[a.text] || 0),
  );
}
  ...
  React.Children.forEach(children, (section) => {
    ...
    React.Children.forEach(el.props.children, (row) => {
      ...
      React.Children.forEach(tr.props.children, (cell) => {
        ...
        headers.push(
          React.Children.toArray(th.props.children)
  ...
  return (
    <div className="app">
      ...
      <header className="header">
        <button
          className="header__brand header__brand--btn"
          onClick={() => setMessages([])}
        ...
        {messages.length === 0 ? (
          <div className="welcome">
            ...
            <p className="welcome__sub">
              Ask me anything about Warwick Prep School — admissions,
              curriculum, events and more.
            </p>
              ...
              <>
                ...
                <div className="welcome__popular">
                  {popular.map((p, i) => (
                    <button
                      key={i}
                      className="welcome__popular-card welcome__popular-card--hot"
            ...
            {popular.length === 0 ? (
              <div className="welcome__popular">
                {sorted.slice(0, 6).map((p, i) => (
                  <button
                    key={i}
                    className="welcome__popular-card"
                    onClick={() => handlePromptClick(p.text)}
                  >
                    <span className="welcome__card-icon">{p.icon}</span>
                    <span className="welcome__card-text">{p.text}</span>
              ...
              <div className="welcome__more">
                ...
                <button
                  className="welcome__more-item welcome__more-item--all"
                    ...
                    {msg.role === "assistant" ? (
                      ...
                      i === messages.length - 1 ? (
                        ...
                        <ReactMarkdown
                          ...
                          components={{ table: ResponsiveTable as any }}
                        >
                          {msg.content}
                        </ReactMarkdown>
                      )
      ...
      <p className="disclaimer">
        Ask Warwick uses AI and may occasionally be wrong. Always verify
        important information directly with the school.
      </p>
              ...
              <div className="drawer__list">
                {sorted.map((p, i) => (
                  ...
                  >
                    ...
                    {counts[p.text] > 0 && (
                      <span className="drawer__item-count">
                        {counts[p.text]}×
                      </span>
                    )}
                  </button>
